...
type DottedGlowBackgroundProps = {
  ...
  speedScale?: number;
  /** optional callback invoked with the rendered canvas at a capped frame rate */
  onFrame?: (canvas: HTMLCanvasElement) => void;
  /** callback frame rate cap for onFrame (0 disables callback) */
...
export const DottedGlowBackground = ({
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      if (onFrame && frameCallbackFps > 0) {
        const minFrameInterval = 1000 / frameCallbackFps;
        if (now - lastFrameCallbackAt >= minFrameInterval) {
